var Kline = function (option) {
    ...
    this.reverseColor = true;//true:红涨绿跌 false:绿涨红跌
    this.isSized = false;
    this.paused = false;
...
Kline.prototype = {
    ...
    onKlineChange: function (data) {
        if (this.debug) {
            console.log(`DEBUG: kline data changed to ${data}`);
        }
    },
    onOrderChange: function (data) {
        if (this.debug) {
            console.log(`DEBUG: order data changed to ${data}`);
        }
    }
};
...
MAIndicator.prototype.__construct = function () {
    ...
    var M3 = new ParameterExpr("M3", 2, 1000, 30);
...
DarkTheme.prototype.__construct = function () {
    ...
    this._colors[Theme.Color.Background] = "#262830";
...
function requestSuccessHandler(res) {
    ...
    KlineIns.onKlineChange(KlineIns.data);
